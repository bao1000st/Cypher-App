...
class Des:
    ...
    def des(self, msg, rkb, rk):
        final = ""
        for i in range(ceil(len(msg) / 16)):
            pt = msg[0 + 16 * i : 16 + 16 * i]
            pt = hex2bin(pt)
    ...
    def make_key(self, key):
        key = ("".join([hex(ord(i))[2:] for i in key])).upper()
        # Sinh khóa
        key = hex2bin(key)
    ...
    def encrypt(self, msg, key):
        if len(msg) % 8 != 0:
            add_part = abs(ceil(len(msg) / 8) * 8 - len(msg))
            msg += "".join("." for i in range(add_part))
        msg = ("".join([hex(ord(i))[2:] for i in msg])).upper()

        rkb, rk = self.make_key(key)

        text = self.des(msg, rkb, rk)
    ...
    def decrypt(self, cipher_text, key):
        ...
        text = self.des(cipher_text, rkb_rev, rk_rev)
        result = "".join(
            [chr(int(text[i] + text[i + 1], 16)) for i in range(0, len(text), 2)]
        )
        return result
